...
| `SCR-011 Maintenance Configuration S01` | `CANDIDATE_READY` |
| Próxima pantalla tras validar SCR-011 | `SCR-012 Risk Profile / Matrix Configuration` |
...
| 03 | SCR-011 | Project Setup | **Maintenance Configuration** | Definir cómo se comporta CMMS para el proyecto. | Corporate Default vs Project Override vs Project Specific; opciones de ingeniería y governance; draft/version; reset/inheritance. | Project Profile; Shared UI Foundation. | Gate Studio del candidato S01. | ALTA | `CANDIDATE_READY` | Validar S01 en Studio. |
| 04 | SCR-012 | Project Setup | **Risk Profile / Matrix Configuration** | Configurar y versionar el modelo de riesgo consumido por Criticality y AMEF. | Dimensiones; niveles; rangos; bandas; colores semánticos; reglas; overrides; preview; versionado; no hardcodear 5×5. | Project Profile; Maintenance Configuration; Shared UI Foundation. | Debe demostrar dos perfiles estructuralmente distintos con la misma UI. | ALTA | `READY_TO_BUILD` | Ejecutar `CREATE_NEW_SCREEN` tras PASS de SCR-011. |
