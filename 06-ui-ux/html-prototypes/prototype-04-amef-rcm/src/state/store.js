(function(){
  var STORAGE_KEY="cmms2.prototype04.state.v1";
  var clone=function(value){return JSON.parse(JSON.stringify(value));};
  var state=clone(window.CMMS_INITIAL_STATE);
  function load(){
    try{
      var saved=localStorage.getItem(STORAGE_KEY);
      if(saved){var parsed=JSON.parse(saved);state=Object.assign(clone(window.CMMS_INITIAL_STATE),parsed);state.uiState=Object.assign(clone(window.CMMS_INITIAL_STATE.uiState),parsed.uiState||{});}
  ...
  window.CMMS_STORE={load:load,get:get,setStep:setStep,save:save,reset:reset,storageKey:STORAGE_KEY};
})();
