(function(){
  ...
  function render(){
    state=store.get();renderStepper();renderContext();state.uiState.currentStep===0?renderLanding():renderStep(steps[state.uiState.currentStep]);renderSaveState();bindDynamic();document.title="CMMS 2.0 · "+steps[state.uiState.currentStep].title;
  }
  ...
  function goTo(step){if(step<0||step>=steps.length)return;store.setStep(step);closeMenu();render();el("mainContent").focus();}
